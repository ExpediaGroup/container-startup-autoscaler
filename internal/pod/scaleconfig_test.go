...
func TestScaleConfigStoreFromAnnotations(t *testing.T) {
	...
	tests := []struct {
		name           string
		configMockFunc func(*podtest.MockKubeHelper)
		want           *want
		wantErrMsg     string
	}{
		{
			name: "UnableToGet" + podcommon.AnnotationTargetContainerName,
			configMockFunc: func(m *podtest.MockKubeHelper) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			config := &scaleConfig{kubeHelper: podtest.NewMockKubeHelper(tt.configMockFunc)}
